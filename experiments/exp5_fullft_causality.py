#!/usr/bin/env python3
"""
Exp 5: Full Fine-Tuning Causality — PR Collapse → Benchmark Degradation
========================================================================
The definitive causal experiment. Full FT (no LoRA shielding) on Qwen3-0.6B,
evaluating standard benchmarks (ARC-Easy, HellaSwag) at each checkpoint
to establish a hard link between PR collapse and measurable capability loss.

Why full FT:
  - LoRA only touches 0.19% of params → PR collapse is "shielded" from outputs
  - Full FT means PR collapse DIRECTLY affects every weight → should show in benchmarks

Why lm-eval:
  - Industry-standard benchmarks (not custom generation metrics)
  - Reviewers cannot dismiss ARC/HellaSwag drops as "soft" metrics

Design:
  - Qwen3-0.6B (fits in 24GB with full FT + gradient checkpointing)
  - DPO for 800 steps, saving checkpoints every 100 steps
  - PR measured every 25 steps via SpectralProbeCallback
  - At each checkpoint: run lm-eval on arc_easy + hellaswag
  - Compute Spearman correlations between PR trajectory and benchmark scores

Usage:
    CUDA_VISIBLE_DEVICES=7 python experiments/exp5_fullft_causality.py
"""
import json
import os
import sys
...
log = setup_logging("exp5_fullft")

MODEL_PATH = "/cache/zhangjing/models/Qwen3-0.6B"
RESULTS_DIR = Path(__file__).resolve().parent / "results" / "exp5_fullft_causality"
...
# ── Load model for full FT (no LoRA) ────────────────────────────
...
class FullModelCheckpointCallback(TrainerCallback):
    """Save full model at regular intervals."""
...
# ── Benchmark evaluation via lm-eval ────────────────────────────

def run_lm_eval(model, tokenizer, tasks: list[str],
                batch_size: int = 16) -> dict:
    """Run lm-eval-harness benchmarks on the model in-process."""
    import lm_eval
...
def main():
    t0 = time.time()
    log.info("=" * 60)
    log.info("Exp 5: Full Fine-Tuning Causality (PR → Benchmark)")
    ...
    # ── Phase 1: Full FT DPO training (2-GPU: model on 0, ref on 1) ──
    log.info("--- Phase 1: Full FT DPO Training (train=GPU0, ref=GPU1) ---")
    ...
    probe_cb = SpectralProbeCallback(
        every_n=PROBE_EVERY, n_samples=30, tag="fullft_causality"
    )

    config = DPOConfig(
        output_dir=str(RESULTS_DIR / "training"),
        run_name="exp5_fullft_causality",
        max_steps=MAX_STEPS,
    ...
    # ── Phase 2: Evaluate each checkpoint with lm-eval ──────────
    ...
    # Also evaluate the final model (save + eval if not already captured)
    ...
    save_result(result, RESULTS_DIR / "fullft_causality_results.json")

    # ── Phase 4: Plot ───────────────────────────────────────────
    try:
        plot_fullft_causality(all_checkpoints, correlations, probe_cb.history,
                              baseline_pr)
    except Exception as e:
        log.error("Plot failed: %s", e, exc_info=True)

    elapsed = (time.time() - t0) / 60
    log.info("=== Exp 5 DONE in %.1f min ===", elapsed)


def plot_fullft_causality(checkpoints: list[dict], correlations: dict,
                          pr_history: list[dict], baseline_pr: float):
    import matplotlib
    matplotlib.use("Agg")
    ...
    fig.suptitle(
        "Exp 5: Full Fine-Tuning Causality — PR Collapse → Benchmark Degradation\n"
        f"Model: Qwen3-0.6B | Method: Full FT DPO | Steps: {MAX_STEPS}",
    ...
    # Top-left: PR trajectory (dense from probe)
    ax = axes[0, 0]
    if pr_history:
    ...
    ax.set_title("PR Trajectory (Full FT)")
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)

    # Top-right: Benchmark scores over steps
    ax = axes[0, 1]
    ...
    # Bottom: PR vs each benchmark (scatter with correlation)
    for i, task in enumerate(BENCHMARK_TASKS):
        ax = axes[1, i]
        task_prs = []
        task_scores = []
    ...
    plt.tight_layout()
    fig_path = RESULTS_DIR / "fig_fullft_causality.png"
